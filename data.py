...
class Dictionary:
    ...
    def read(self, path):
        with open(path + ".words.txt", 'r', encoding='utf8') as f:
            if self.char:
                chars = set(f.read())
...
class Data:
    ...
    def read(self, path, dictionary):
        with open(path, 'r', encoding='utf8') as f:
            ...
            for line in f:
                ...
                # if not fields:
                #     continue
                if fields and fields != fields2 and fields[0] != '#':
                    # print('f1', fields)
                    # print('f2', fields2)
                    fields = fields2
                if fields:
                    ...
                    ws, ts, hs, ls, n = self.newline()
            # print(self.words, self.tags, self.heads, self.labels, self.lengths)
    ...
    def order(self):
        ...
        # print(*sorted(old_order, key=lambda t: t[1])[len(self.lengths)-1][0])
        # print(len(self.lengths), self.lengths, sorted(old_order, key=lambda t: t[1]), list(zip(*sorted(old_order, key=lambda t: t[1]))))
        new_order = zip(*sorted(old_order, key=lambda t: t[1]))
        # print(list(new_order))
        new_order = list(new_order)[0]
        # print(self.words[0], 'joe', list(new_order)[1])
        # print('joe******************************', sorted(old_order, key=lambda t: t[1]))
...
if __name__ == "__main__":
    ...
    from embedding import RecurrentCharEmbedding
